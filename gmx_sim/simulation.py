...
class SimFiles:
    ...
    def add_files(self, names: str) -> str:
        ...
        for file_name in names:
            ...
            file_paths.append((file_path, file_ending))
        # check if all paths exist, if such, signal not to run
        if np.all([os.path.exists(file_path[0]) for file_path in file_paths]):
            return None
        # if some exist, back them up. Run through all and add them to self.files
        for file_path, file_ending in file_paths:
        ...
        # file_paths[:][1] contains file ending
        return [x[0] for x in file_paths]
    ...
    def get_latest_by_type(self, file_ending):
        """ Given a file ending, return the latest file of that type stored. """

        if self.files[file_ending] == []:
            raise ValueError(f'No files generated yet for ending {file_ending}.')
        return self.files[file_ending][-1]
...
class Simulation:
    ...
    def add_to_gmx_kwargs(self, gmx_kwargs: dict, command_str: str) -> None:
        """ Split into commdands (start with -), add each command with its id
        (what follows the -) into gmx_kwargs with its body as string. If no body
        add True as body. """

        command_parsed = [shlex.split(x) for x in '-c -bt cubic -d 10'.split('-') if x]
        for command in command_parsed:
            if len(command) == 1:
                gmx_kwargs[command[0]] = True
            else:
                gmx_kwargs[command[0]] = ' '.join(command[1 : ])
    ...
    def pdb2gmx(self) -> None:
        ...
        if file_paths is None:
            ...
            gmx_kwargs = dict(zip(["o", "p", "i"], file_paths))
    ...
    def gen_box(self, vacuum=False):
        """ Generate a box based on the config. For the vacuum minimization,
        we need a very big box, to model vacuum. """

        # check newest input file before adding files
        in_file = self.files.get_latest_by_type('gro')
        gmx_kwargs = {}
        if vacuum is True:
            out_file = self.files.add_files(['box_vac.gro'])[0]
            self.add_to_gmx_kwargs(gmx_kwargs, '-c -bt cubic -d 1000')
        else:
            out_file = self.files.add_files(['box.gro'])[0]
            self.add_to_gmx_kwargs(gmx_kwargs, self.config['general']['box_params'])
        if out_file is None:
            LOGGER.info(f"Found output for gen_box, {vacuum=} already, skipping.")
        self.files.set_output_file(f"gen_box_vac_{vacuum}")
        gromacs.editconf(
            f=in_file,
            o=out_file,
            **gmx_kwargs,
